...
func postCreate(t *testing.T, h *AccountHandler, user *domain.User, name string) *httptest.ResponseRecorder {
	t.Helper()
	form := url.Values{"name": {name}, "expires": {"90d"}}
	r := httptest.NewRequest(http.MethodPost, "/account/tokens", strings.NewReader(form.Encode()))
...
func postRevoke(t *testing.T, h *AccountHandler, user *domain.User, id string) *httptest.ResponseRecorder {
	t.Helper()
	r := httptest.NewRequest(http.MethodPost, "/account/tokens/"+id+"/revoke", nil)
...
func TestTheCreateResponseShowsThePlaintextExactlyOnce(t *testing.T) {
	...
	h := NewAccountHandler(services.NewTokenService(dbadapter.NewTokenRepo(db), dbadapter.NewUserRepo(db)))
...
// The voluntary password flow: from=account must return the person to the
// account page in both directions — inline errors on refusal, a redirect
// back to the tab on success — while the forced flow keeps the standalone
// page it always had.
func TestAVoluntaryPasswordChangeStaysOnTheAccountPage(t *testing.T) {
	db := tokenHandlerDB(t)
	repo := dbadapter.NewUserRepo(db)
	hash, _ := bcrypt.GenerateFromPassword([]byte("old-password"), bcrypt.MinCost)
	user := &domain.User{
		Name: "Pw Test", Email: "pw-" + uuid.NewString() + "@example.com",
		PasswordHash: string(hash), Role: domain.RoleMember,
	}
	if err := repo.Create(context.Background(), user); err != nil {
		t.Fatalf("seed user: %v", err)
	}
	...
	h := NewAuthHandler(services.NewAuthService(repo))
	...
	// Refused: the account page renders with the error inline, not the
	// standalone forced-change page.
	refused := post("not-the-password", "new-password-1")
	if !strings.Contains(refused.Body.String(), "account-body") {
		t.Error("a refused change should render the account page")
	}
	if !strings.Contains(refused.Body.String(), "That is not your current password") {
		t.Error("the refusal should carry its field error")
	}

	// Accepted: back to the tab, with the fresh session cookie set.
	accepted := post("old-password", "new-password-1")
	if accepted.Code != http.StatusSeeOther {
		t.Fatalf("status = %d, want 303", accepted.Code)
	}
	if got := accepted.Header().Get("Location"); got != "/account?tab=password&saved=1" {
		t.Errorf("redirected to %q, want the password tab", got)
	}
	if !strings.Contains(accepted.Header().Get("Set-Cookie"), "cs_session=") {
		t.Error("the replacement session cookie is missing")
	}
}
