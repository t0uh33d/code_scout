...
// raw sort by with namespace support
...
// Validate sorting validates the sorting and return if the user is allowed to perform the sort
//
// Parameters:
//   - sort: string (Sort being validated)
...
// Validate filters validates the filters and return if the user is allowed to perform the filter
//
// Parameters:
//   - filter: Filter (Filter being validated)
//   - isRawQuery: bool (If the query is raw or not)
